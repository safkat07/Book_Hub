      ...
      <div className="flex items-center   justify-center">
        <div className="group relative  rounded-tl-[1.9rem] rounded-xl XL:hover:scale-105  transition-all duration-1000 rounded-bl-[1.9rem]    cursor-pointer items-center justify-center overflow-hidden hover:shadow-xl hover:shadow-black/30">
          ...
          <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
            ...
            <p className="mb-1 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">{bookCategory}</p>
            <p className="mb-1 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">{authorName}</p>
            <p className="mb-1 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">{quantity} Books Available</p>
            <p className="mb-1 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">Language: {bookLanguage}</p>
            <ReactStars
              count={rating}
              value={rating}
              size={24}
